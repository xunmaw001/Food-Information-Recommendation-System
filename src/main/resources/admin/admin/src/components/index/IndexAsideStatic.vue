<template>
  <el-aside class="index-aside" height="100vh" width="210px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:var(--publicSubColor)">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%; background-color:var(--publicSubColor);text-color:#ffffff ;active-text-color:#FFFFFF" default-active="0">
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "var(--publicMainColor)"
          })
          el.addEventListener("mouseleave", e => {
            e.stopPropagation()
            el.style.backgroundColor = "var(--publicSubColor)"
          })
